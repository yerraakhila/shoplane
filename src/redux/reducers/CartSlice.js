...
export const CartSlice = createSlice({
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      let obj = state.cartWithUserInfo.find(
        (eachObj) => eachObj.user === currentUser
      );
      if (obj) {
        let cartItem = obj.cartInfo.cartItemsList.find(
    ...
    deleteFromCart(state, action) {
      ...
      if (obj) {
        let index = obj.cartInfo.cartItemsList.findIndex(
    ...
    clearCart(state) {
      ...
      let obj = state.cartWithUserInfo.find((o) => o.user === currentUser);
      if (obj) {
        obj.cartInfo.cartItemsList = [];
        obj.cartInfo.cartItemsNum = 0;
      }
    }
...
export const { addToCart, deleteFromCart, decrementFromCart, clearCart } = CartSlice.actions;
